 ...
 * @version    $Id: Tstudent.php 327 2010-04-19 09:32:50Z dbojdo $
 */
 ...
 *
 */
class App_R_Function_Tstudent extends App_R_Function {
	public function init() {
		$this->_viewScript = $this->_resultScript = 'advance/tstudent.phtml';
		$this->addFile('common/homogeneity.R');
		$this->addFile('common/srednie_fn.R');
		$this->addFile('common/tstudent.R');
	}
	
	protected function _parseResult() {
		parent::_parseResult();
		
		if(!file_exists($this->getTask()->getTaskPath() . DS . 'tstudent.csv')) {
			$this->_result->errors[] = 'Nie znaleziono pliku wynikowego: tstudent.csv';
		} else {
			$f = fopen($this->getTask()->getTaskPath() . DS . 'tstudent.csv','r');
