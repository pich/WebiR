 ...
 * @version    $Id: Wilcoxon.php 327 2010-04-19 09:32:50Z dbojdo $
 */
 ...
 *
 */
class App_R_Function_Wilcoxon extends App_R_Function {
	public function init() {
		$this->_viewScript = $this->_resultScript = 'advance/wilcoxon.phtml';
		$this->addFile('common/srednie_fn.R');
		$this->addFile('common/wilcoxon.R');
	}
	
	public function _parseResult() {
		parent::_parseResult();
		
		if(!file_exists($this->getTask()->getTaskPath() . DS . 'wilcoxon.csv')) {
			$this->_result->errors[] = 'Nie znaleziono pliku wynikowego: wilcoxon.csv';
		} else {
			$f = fopen($this->getTask()->getTaskPath() . DS . 'wilcoxon.csv','r');
